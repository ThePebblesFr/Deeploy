...
class ES_CNN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(1, 8, kernel_size=(3, 1), padding=(1, 0), bias=True)
        self.conv2 = nn.Conv2d(8, 16, kernel_size=(3, 1), padding=(1, 0), bias=True)

        self.pool = nn.MaxPool2d((2, 1), (2, 1))

        # input: 1 x 16 x 1
        # after pool1: 8 x 8 x 1
        # after pool2: 16 x 4 x 1
        self.fc1 = nn.Linear(16 * 4 * 1, 32, bias=True)
        self.fc2 = nn.Linear(32, num_classes, bias=True)

    def forward(self, x):
        # expect input shape: (N, 1, 16, 1)
...
class TinyImageClassification(nn.Module):

    def __init__(self, num_classes=10):
        super().__init__()

        self.conv1 = nn.Conv2d(3, 8, kernel_size=3, padding=1, bias=True)
        self.conv2 = nn.Conv2d(8, 12, kernel_size=3, padding=1, bias=True)
        self.conv3 = nn.Conv2d(12, 12, kernel_size=3, padding=1, bias=True)

        self.pool = nn.MaxPool2d(2, 2)

        # 32x32 -> pool -> 16x16 -> pool -> 8x8
        self.fc = nn.Linear(12 * 8 * 8, num_classes, bias=True)

    def forward(self, x):
        x = F.relu(self.conv1(x))   # 8 x 32 x 32
        x = self.pool(x)            # 8 x 16 x 16

        x = F.relu(self.conv2(x))   # 12 x 16 x 16
        x = self.pool(x)            # 12 x 8 x 8

        x = F.relu(self.conv3(x))   # 12 x 8 x 8
...
class SimpleMLP(nn.Module):
    ...
    def __init__(self, input_dim=392, hidden_dim=64, output_dim=10):
        super().__init__()
